<template>
  <nav class="z-20 bg-blue-800 fixed inset-x-0 top-0 shadow-md">
          ...
          <div class="hidden lg:block lg:ml-auto">
            <div class="flex space-x-1">
              <div v-for="route in routes" :key="route.path">
                <router-link
                  v-if="route.isTopHeader"
                  class="text-white hover:bg-blue-900 focus:outline-none focus:bg-blue-900 py-2 px-4 rounded"
                  :to="route.path"
    ...
    <div class="lg:hidden" :class="openMenu ? 'hidden' : 'block'">
      <div class="px-2 pt-2 pb-3 flex flex-col space-y-1" style="height: 100vh">
        <div v-for="route in routes" :key="route.path">
          <router-link
            v-if="route.isTopHeader"
            class="block px-3 py-2 rounded-md text-white hover:text-white hover:bg-blue-900 focus:outline-none focus:text-white focus:bg-blue-900 transition duration-150 ease-in-out"
            :to="route.path"
          >{{ route.name }}</router-link>
        </div>
      </div>
...
<script>
import routes from "@/router/guest.routes.js";
